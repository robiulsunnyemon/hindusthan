...
def create_schema_files(main_folder, model_name, fields):
    ...
    # Response schema (include auto fields + auth fields)
    response_fields = [
        '    id: str',
...
async def get_{snake_name}(id: str):
    ...
    Get {snake_name} by ID
    {chr(34)}{chr(34)}{chr(34)}
    {snake_name} = await {model_name}Model.get(id)
    if not {snake_name}:
...
@router.patch("/{{id}}", response_model={camel_name}Response,status_code=status.HTTP_200_OK)
async def update_{snake_name}(id: str, {snake_name}_data: {camel_name}Update):
    {chr(10)}    {chr(34)}{chr(34)}{chr(34)}
    ...
    return await {model_name}Model.get(id)
